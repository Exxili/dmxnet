...
declare class sender {
  /**
   * Creates a new sender, usually called through factory in dmxnet
   *
...
declare class receiver extends EventEmitter {
  /**
   * Creates a new receiver, usually called through factory in dmxnet
   *
...
declare class dmxnet {
  ...
  socket_ready: boolean;
  /**
   * Listens for ArtNet Sync packets
   *
   * @param {function} callback - Callback to handle incoming ArtNet Sync packets
   */
  sync(
    callback: (data: { ip: string; port: number; message: Buffer }) => void
  ): void;
  /**
